...
export async function fetchProjectBundle(projectId: string) {
  const [projectResponse, boardResponse, applicationsResponse, statsResponse, tasksResponse, foldersResponse] =
    await Promise.all([
      apiRequest<ApiDataResponse<ApiProject>>(`/projects/${projectId}`),
      apiRequest<ApiDataResponse<ApiEditorBoard | null>>(`/projects/${projectId}/editor-boards`).catch(() => ({ data: null })),
      ...
      apiRequest<ApiDataResponse<{ metrics?: Record<string, unknown> }>>(`/projects/${projectId}/stats`).catch(() => ({ data: undefined })),
